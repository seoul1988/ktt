import Link from "next/link";import { supabase } from "../../lib/supabase";import CommunityBottomNav from "../components/CommunityBottomNav";import ProfileButton from "../components/ProfileButton";import InquiryTab from "../components/InquiryTab";import AdTab from "../components/AdTab";

export const dynamic = "force-dynamic";export const revalidate = 0;

export default async function CommunityPage() {const today = new Date().toISOString().slice(0, 10);

const { data: events, error: eventsError } = await supabase.from("community_events").select("*").order("created_at", { ascending: false }).limit(1);

if (eventsError) {console.error("community events error:", eventsError);}

const { data: deals, error: dealsError } = await supabase.from("deals").select("*").eq("status", "approved").eq("active", true).eq("deal_scope", "community").or(end_date.is.null,end_date.gte.${today}).order("created_at", { ascending: false }).limit(6);

if (dealsError) {console.error("community deals error:", dealsError);}

const { data: allBusinesses } = await supabase.from("businesses").select("*").order("created_at", { ascending: false });

const { data: categories } = await supabase.from("categories").select("name, show_on_community_map");

const communityCategoryNames = new Set((categories || []).filter((cat) => cat.show_on_community_map === true).map((cat) => String(cat.name).trim().toLowerCase()));

const newBusinesses =allBusinesses?.filter((biz) => {const bizCategories = String(biz.category || "").split(",").map((cat) => cat.trim().toLowerCase()).filter(Boolean);

    return bizCategories.some((cat) => communityCategoryNames.has(cat));
  })
  .slice(0, 6) || [];

const { data: featured } = await supabase.from("featured_businesses").select("*").eq("active", true).order("created_at", { ascending: false }).limit(3);

const eventCount = events?.length || 0;const dealCount = deals?.length || 0;

return (<main className="min-h-screen bg-[#F8F3EC] text-[#172033]"><section className="mx-auto max-w-xl px-5 pb-28 pt-6"><div className="mb-6 flex items-start justify-between gap-4">
