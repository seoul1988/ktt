...
export default function TodaysKoreaNewsModal({
  ...
  const activePosts = activeType === "kpop" ? kpopNews : kdramaNews;
  ...
  function moveModal(direction: -1 | 1) {
    if (selectedIndex < 0 || allPosts.length < 2) return;
  ...
  function scrollSlider(direction: -1 | 1) {
    ...
    const amount = card ? card.offsetWidth + 12 : slider.clientWidth * 0.8;
  ...
  function handleSliderScroll() {
    ...
    setActiveSlide(Math.max(0, Math.min(nextIndex, activePosts.length - 1)));
  }
  ...
  <div className="flex items-center justify-between gap-3">
    <div className="min-w-0">
      <h3 className="text-[18px] font-black leading-none text-[#172033]">
        🌸 Today's Korea
      </h3>
                  ...
                  {activePosts.map((post) => (
                    ...
                    >
                      <div className="relative h-[120px] w-full overflow-hidden rounded-xl bg-gray-100">
                        ...
                        <span className="absolute left-1.5 top-1.5 rounded bg-violet-600 px-1.5 py-0.5 text-[7px] font-black text-white">
                          NEW
                        </span>
                      </div>

                      <h5 className="mt-2 line-clamp-2 min-h-[42px] text-[12px] font-black leading-[1.4] text-[#172033]">
                        {post.title}
                      </h5>

                      <p className="mt-1 text-[10px] font-semibold text-gray-400">
                        {formatNewsDate(post.published_at) || "Latest"}
                      </p>
                    </button>
                  ))}
                </div>
                ...
                {activePosts.length > 1 && (
                  <>
                    <button
                      ...
                      aria-label="이전 뉴스"
                      className="absolute left-1 top-[42px] z-10 hidden h-8 w-8 items-center justify-center rounded-full bg-white/95 text-lg font-black text-gray-700 shadow-md sm:flex"
                    >
                      ‹
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => scrollSlider(1)}
                      aria-label="다음 뉴스"
                      className="absolute right-0 top-[42px] z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white/95 text-lg font-black text-gray-700 shadow-md"
                    >
                      ›
                    </button>
                  </>
                  ...
                  {activePosts.map((post, index) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        slider.scrollTo({
                          left: index * (card.offsetWidth + 12),
                          behavior: "smooth",
                        });
                      }}
  ...
  onClick={closeModal}
  aria-label="닫기"
  className="absolute right-3 top-3 z-30 flex h-9 w-9 items-center justify-center rounded-full border border-white/40 bg-black/65 text-2xl font-black leading-none text-white shadow-lg backdrop-blur"
>
  ×
</button>
            ...
            <div className="relative aspect-video w-full overflow-hidden rounded-t-[28px] bg-gray-100">
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent px-5 pb-4 pt-14">
                <span className="rounded-full bg-white/95 px-3 py-1 text-[11px] font-black text-[#172033]">
              ...
              {allPosts.length > 1 && (
                <>
                  <button
                    ...
                    onClick={() => moveModal(-1)}
                    aria-label="이전 기사"
                    className="absolute left-3 top-1/2 z-20 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/55 text-2xl font-black text-white shadow-lg"
                  >
                    ‹
                  </button>
                  ...
                  <button
                    ...
                    onClick={() => moveModal(1)}
                    aria-label="다음 기사"
                    className="absolute right-3 top-1/2 z-20 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/55 text-2xl font-black text-white shadow-lg"
                  >
                    ›
                  </button>
                </>
            ...
            <div className="p-5 sm:p-6">
              <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold text-gray-500">
                <span>{selectedPost.source_name || "Source"}</span>

                {formatNewsDate(selectedPost.published_at) && (
                  <>
                    <span>•</span>
                    <span>{formatNewsDate(selectedPost.published_at)}</span>
                  </>
                )}
              ...
              <h3 className="mt-3 break-words text-[22px] font-black leading-tight text-[#172033]">
                {selectedPost.title}
              </h3>
  ...
  <button
    ...
    aria-label="모달 닫기"
    title="닫기"
    className="flex h-12 w-12 items-center justify-center rounded-full border border-gray-300 bg-white text-2xl font-black text-[#172033] shadow-sm transition active:scale-95"
  >
    ×
  </button>
