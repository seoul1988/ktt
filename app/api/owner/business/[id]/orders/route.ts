...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await db
      ...
      .select("id,order_number,fulfillment_type,customer_name,customer_phone,requested_time,payment_method,payment_status,order_status,subtotal,tax,tip,total,delivery_address,created_at,restaurant_order_items(id,item_name,quantity,unit_price,line_total,instructions)")
